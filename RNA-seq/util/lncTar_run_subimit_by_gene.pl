...
sub run
{
	...
	foreach my $trans (keys %target_mRNA) {
		...
		# run lncTar
		# print qq{perl /media/gsadmin/vd3/xudongliang/software/LncTar/LncTar.pl -p 1 -l $temp_out/$trans.$random_word.input -m $temp_out/$trans.$random_word.mRNA.input  -d -0.1 -s F -o $temp_out/$trans.target.txt > $temp_out/$trans.log 2>& 1\n};
		system qq{perl /media/gsadmin/vd3/xudongliang/software/LncTar/LncTar.pl -p 1 -l $temp_out/$trans.$random_word.input -m $temp_out/$trans.$random_word.mRNA.input  -d -0.1 -s F -o $temp_out/$trans.target.txt > $temp_out/$trans.log 2>& 1\n};
		system qq{rm -f /tmp/$trans.$random_word.mRNA.input_mRNA_temp.txt};
		system qq{rm -f /tmp/$trans.$random_word.input_lncRNA_temp.txt};
