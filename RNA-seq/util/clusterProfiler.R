...
eg <- bitr(gene, fromType="ENSEMBL", toType=c("ENTREZID","SYMBOL"), OrgDb=db[[organ]])
gene     <- eg[[2]]
...
ensembl2symbol  <- paste(prefix, "ID.convert.xls", sep="/")
write.table(eg, ensembl2symbol, sep = "\t", row.names = F, quote = F)
...
kk_enrich$geneID <- unlist(lapply(1:length(kk_enrich$geneID), function(t){  paste(eg[[3]][eg[[2]] %in% unlist(strsplit(kk_enrich$geneID[t], "/"))], collapse = "/")  }))
