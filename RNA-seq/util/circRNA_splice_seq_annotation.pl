...
sub parse_gtf
{
	...
	while (<GTF>) {
	    ...
	    push @{$hash{$trans}}, qq{$start:$end:$chr:$gene:$strand};
	}
	close GTF;
	...
	foreach my $x (keys %hash) {
    	my $exon_num = 0;
    	my $end_pos = 0;
    	foreach my $y (sort {(split/:/,$a)[0] <=> (split/:/,$b)[0] } @{$hash{$x}}) {
        	$exon_num++;
        	my ($start, $end, $chr, $gene, $strand) = split /:/, $y;
        	my $name    = qq{$x|exon_$exon_num|$gene};
        	if ($exon_num > 1 and $exon_num <= scalar @{$hash{$x}}) {
            	my $intron_num = $exon_num - 1;
            	my $name    = qq{$x|intron_$intron_num|$gene};
            	print BED qq{$chr\t$end_pos\t$start\t$name\t.\t$strand\n};
        	}
...
sub cal_overlap
{
	system qq{/media/gsadmin/vd1/heguangliang/bcbio3/thirdTools/bin/bedtools intersect  -a $bed -b $out_dir/refgene.bed  -wb -s > $out_dir/overlap.xls};
}
